...
- Per-stop timings. Instead, each trip's start timing is taken and duration is take or computed. So in `stop_times.txt`, first and last stops in each trip will have timings (no waiting time at stop) and intermediate stops' arrival and departure times are left blank.
